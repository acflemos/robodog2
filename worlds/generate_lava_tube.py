#!/usr/bin/env python3
"""Gera worlds/lava_tube.world — lava tube lunar em caixa oca (v1 aprovada).

Estratégia:
  - Segmentos retos: piso + teto + paredes laterais com espessura visível
  - 6 segmentos em curva S + alcova + câmara final com enigma
  - 100% primitivas (box/cylinder/sphere), leve para VM 16 GB
"""
...
def hollow_segment(name, cx, cy, yaw, length, width=INNER_W, height=INNER_H, end_wall=False):
    half_w = width / 2
    lines = [
        f"    <model name=\"{name}\">",
        "      <static>true</static>",
...
def rubble(name, x, y, z, yaw, size, pose_z):
    sx, sy, sz = size
    return [
        f"    <model name=\"{name}\">",
        "      <static>true</static>",
...
def artifacts():
    lines = []
    for args in (
        ("rubble_0", 49, 7, 0, 0.0, (1.2, 0.8, 0.4), 0.2),
        ("rubble_1", 51, 6, 0.15, 0.3, (0.9, 0.6, 0.35), 0.175),
        ("rubble_2", 50, 8, 0.25, 0.6, (0.7, 0.7, 0.5), 0.25),
        ("rubble_3", 52, 7.5, 0.1, 0.9, (1.0, 0.5, 0.3), 0.15),
    ):
        lines.extend(rubble(*args))
...
def main():
    parts = [
        '<?xml version="1.0" ?>',
        "<!-- lava_tube.world — Lava tube lunar, gravidade 1/6g, otimizado para VM 16GB.",
        "     Câmara final: restos de missão antiga + pirâmide + beacon emissivo.",
        "     Geometria 100% primitiva (box/cylinder/sphere), modelos estáticos.",
        ...
        *tunnel_lights(),
        ...
        "      <link name=\"link\">",
        *box_pair("surface", "30 40 0.2", "0 0 0 0 0 0", "0.20 0.18 0.16 1", "0.25 0.22 0.20 1"),
        "      </link>",
        "    </model>",
        "",
    ]
    ...
    for i, (cx, cy, yaw) in enumerate(SEGMENTS):
        parts.extend(hollow_segment(f"tube_seg_{i:02d}", cx, cy, yaw, SEG_LEN))

    parts.extend(hollow_segment("alcove", 32, 8, 1.5708, 10.0, width=3.0, end_wall=True))
